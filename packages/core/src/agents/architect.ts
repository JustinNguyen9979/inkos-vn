...
export class ArchitectAgent extends BaseAgent {
  ...
  async generateFoundation(
    ...
    const langPrefix = resolvedLanguage !== "zh"
      ? resolvedLanguage === "vi"
        ? `【LANGUAGE OVERRIDE】ALL reader-facing content in story_frame, volume_map, roles, book_rules, and pending_hooks MUST be written in Vietnamese. The === SECTION: === tags remain unchanged. Do NOT emit rhythm_principles or current_state sections — rhythm principles live inside the last paragraph of volume_map; environment/era anchors (when relevant) are woven into story_frame's world-tonal-ground paragraph.\n\n`
    ...
    const userMessage = resolvedLanguage !== "zh"
      ? resolvedLanguage === "vi"
        ? `Generate the complete foundation for a ${gp.name} novel titled "${book.title}". Write all reader-facing content in natural Vietnamese.`
    ...
    const response = await this.chat([
      { role: "system", content: withVietnameseOutputContract(langPrefix + systemPrompt + revisePrompt, resolvedLanguage) },
...
You MUST emit all **5 SECTION blocks in order**: story_frame → volume_map → roles → book_rules → pending_hooks. Do NOT stop after story_frame or volume_map just because they ran long. Even if roles lists only 3 characters, book_rules is a small Markdown block, and pending_hooks has only 3 rows, all five must appear. The output is only considered delivered after the last row of pending_hooks is written.`;
  ...
  // Parsing
  // -------------------------------------------------------------------------
  private async parseSectionsWithRepair(content: string, language: "zh" | "en" | "vi"): Promise<ArchitectOutput> {
    try {
  ...
  private parseSections(content: string, language: "zh" | "en" | "vi"): ArchitectOutput {
  ...
  private buildStoryBibleShim(language: "zh" | "en" | "vi"): string {
    if (language !== "zh") {
  ...
  private buildCharacterMatrixShim(roles: ReadonlyArray<ArchitectRole>, language: "zh" | "en" | "vi"): string {
...
${trimmed}\n`;
  ...
  private normalizePendingHooksSection(section: string, volumeMapRaw: string): string {
    ...
    const language: "zh" | "en" | "vi" = /[\u4e00-\u9fff]/.test(section) ? "zh" : "en";
  ...
  private normalizeDormantSeedStatus(status: string | undefined, language: "zh" | "en" | "vi"): string {
  ...
  private mergeHookNotes(notes: string, seedNote: string, language: "zh" | "en" | "vi"): string {
