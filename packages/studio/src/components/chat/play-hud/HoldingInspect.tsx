        ...
        <div className="min-w-0 space-y-3 px-3 py-3">
          ...
          {lifecycle ? (
            ...
            <div>
              ...
              {row.isFresh ? (
                <p className="mt-1.5 text-[13px] leading-5 text-emerald-300">▲ {tr("刚获得", "Just acquired")}</p>
              ) : row.changeReason ? (
          ...
          {row.meters.length > 0 ? (
            <div className="space-y-1.5">
              <h4 className="text-[13px] leading-5 uppercase tracking-wider text-muted-foreground/60">{tr("属性", "Stats")}</h4>
              {row.meters.map((m) => (
          ...
          {row.relations.length > 0 ? (
            <div className="min-w-0">
              <h4 className="mb-1 text-[13px] leading-5 uppercase tracking-wider text-muted-foreground/60">{tr("牵动", "Connections")}</h4>
          ...
          {row.provenanceTurn != null ? (
            <p className="break-words border-t border-border/30 pt-2 text-[13px] leading-5 text-muted-foreground/70">
              {getAppLanguage() === "vi" ? `Nhận được ở lượt ${row.provenanceTurn}` : tr(`第 ${row.provenanceTurn} 幕 取得`, `Acquired · Turn ${row.provenanceTurn}`)}
            </p>
          ) : null}
        </div>
      </div>
